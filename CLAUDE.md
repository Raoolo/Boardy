...
Tests: `uv run pytest` runs a small set of **invariant guards** in `tests/` (NOT a full suite): tool-registry coherence, guest can't see/run write tools, `_source` never in a tool schema, `update_inventory` rejects negative, `set_sleeve_requirements` atomic on bad input. pytest is a `dev` dependency-group dep (not baked into the prod image). `tests/conftest.py` points the app at a throwaway DB via `BOARDY_DB` and bootstraps the games + sleeve fact tables that `schema.migrate()` leaves to the ETL. Beyond that, validate by smoke-testing a tool (`uv run python -c "from app.tools import sleeve_summary; print(sleeve_summary())"`) or hitting `POST /chat`. Server has no auto-reload; restart manually after Python changes. `web/` is served live.
...
- `app/audit.py` — every write to `games`/`sleeve_requirements`/`sleeve_inventory` logs to `changes`. **Anche i regolamenti** (dal 2026-06-24): `rulebooks._store_rulebook` logga `table='rulebooks'` (`insert`/`replace`, `source`=attore via il param `actor` propagato da `ingest_bytes`/`ingest_pages`/`ingest`) → download/upload tracciabili; `recent_changes(game_name=...)` li include via `row_label`.
...
- `app/rulebooks.py` — pypdf chunking + e5 embeddings + brute-force cosine. Rulebook RAG flow: `find_rulebook` (read, cerca PDF online) → `download_rulebook` (write, scarica+indicizza in un colpo) → `ask_rules` (read, Q&A con citazione pagina). `ingest_rulebook` resta per PDF già su disco. **Storage: il PDF grezzo vive in `rulebooks.pdf_blob` (DB), NON su disco** — `ingest_bytes(game_name, data, source=...)` è il core (lo usano download/upload/auto-hook); `ingest(path)` legge il file e delega; `get_pdf(name)` ri-estrae i bytes. **Validazione regolamenti a 2 livelli** (la lingua era solo un sintomo; il bug vero era indicizzare il manuale del gioco SBAGLIATO — es. un PDF thai di «Trails» sotto «Barrage»). (1) **Game-match (integrità, deterministico) in `download_rulebook`**: un `bgg_filepageid` è accettato solo se appartiene a `bgg_files_api.find_rulebooks(game.bgg_id)`; `override=true` per bypassare. (2) **Gate lingua (usabilità) in `ingest_bytes`**: `classify_rulebook_language(text)` (py3langid) → allowed (EN/IT/ES/DE/FR/PT, env `BOARDY_RULEBOOK_LANGS`) | soft (latine europee NL/PL/…: warning, indicizza) | unusable (script non-latini thai/CJK/…: hard-reject solo se `enforce_lang=True`). `enforce_lang` default True (auto-hook strict); `download_rulebook`/`ingest` locale lo settano False (user-confirmed → warning, non reject). `ingest_bytes` torna `warnings` (il bot deve mostrarli) + un soft-check che avvisa se il nome del gioco non compare nel testo. `ingest_pages` (foto OCR) NON è gated. Risoluzione nome tollerante via `_resolve_game` (exact LOWER → normalizzato non-ambiguo) usata da ingest+search, così nomi con punteggiatura diversa non falliscono. Fonte primaria dei PDF: **`etl/onejour_api`** (1j1ju.com, vedi sotto). **Regolamenti da FOTO** (2026-06-23): `ingest_pages(game, pages, source, pdf_blob, ocr_report)` indicizza pagine già trascritte (condivide `_store_rulebook` con `ingest_bytes`); orchestrazione in `tools.ingest_rulebook_photos` (OCR → ordina → rileva buchi → assembla PDF con Pillow) usando `app/ocr.py`. Schema v10: `rulebooks.ocr_report` (JSON avvisi). **Auto-OCR dei PDF scansionati** (2026-06-24): se `ingest_bytes(..., ocr_fallback=True)` e pypdf non estrae testo (PDF=immagini), `_ocr_pdf_to_pages` rasterizza con **PyMuPDF (fitz)** e manda le pagine all'OCR Gemini (poi gate lingua + check nome sul testo OCR; cap 40 pag.). `ocr_fallback=True` nei flussi user-confirmed (`download_rulebook`, `/rulebooks/upload`, `ingest` locale), `False` nell'auto-hook. **Audit** (2026-06-24): `_store_rulebook` logga in `changes` (vedi `app/audit.py`).
- `app/ocr.py` — OCR delle foto dei regolamenti via **Google Gemini** (`gemini-2.5-flash`, env `GEMINI_API_KEY`/`GEMINI_OCR_MODEL`). DeepSeek **non** legge immagini → la visione è qui e SOLO qui; DeepSeek resta il cervello per chat/Q&A/tag/titoli/`/library/filter`. `transcribe_images(images, game)` fa 1 call per foto (output JSON via `response_schema` Pydantic, retry su 503/429), restituendo testo Markdown **+ descrizione a parole di diagrammi/icone**, numero di pagina stampato, `legibility` e `issues`. **Formati: JPG, PNG, WebP** (max 40 foto, 15 MB/foto). Interfacce: web `POST /rulebooks/upload-photos` (owner-only) col tasto 📎 accanto a Invia (accetta **PDF o foto**; il PDF va a `/rulebooks/upload`, le foto a `/rulebooks/upload-photos`) + drag-drop; Telegram via didascalia «regolamento di X» o `/regolamento <gioco>`+`/fine`. **`transcribe_images` è usato anche per l'auto-OCR dei PDF scansionati** (vedi `rulebooks._ocr_pdf_to_pages`), non solo per le foto. Vedi LEARNINGS 2026-06-23.
- `etl/onejour_api.py` — wrapper read-only su 1jour-1jeu.com (`/rules/search?q=`, HTML scraping deterministico). Restituisce link PDF diretti `cdn.1j1ju.com` con lingua dedotta dal filename. **Fonte primaria** (no browser, no auth) di `find_rulebook`. **Filtro rilevanza** (2026-06-24): `find_rulebook._candidate_matches_game` scarta i candidati 1j1ju fuzzy-junk (il core-title deve contenere tutti i token del nome gioco) — altrimenti la junk riempiva la lista e *sopprimeva* il fallback web. **Tavily** ora parte quando manca un candidato *usabile* (lista vuota O solo lingua non ammessa, es. solo un PDF giapponese su BGG), non solo a lista vuota; i risultati deterministici + web sono **mergiati**+deduplicati.
...
- `LLM_TEMPERATURE` — sampling temp for the agentic chat loop (`app/llm.py`, default **0**). Set explicitly because the chat loop previously inherited the provider default (DeepSeek/Anthropic = 1.0) — too high for a tool-router. The deterministic sub-tasks (tags/OCR/titles/library filter) hardcode their own T=0 regardless.
- `LIBRARY_FILTER_MODEL` — override the DeepSeek model used by `/library/filter` (default `deepseek-chat`).
- `GEMINI_API_KEY` — Google AI Studio key (https://aistudio.google.com/apikey, free tier). Usata per l'OCR via visione (`app/ocr.py`: DeepSeek non legge immagini): FOTO dei regolamenti **e** auto-OCR dei PDF scansionati (`rulebooks._ocr_pdf_to_pages`). Senza, tutto il resto funziona; falliscono solo l'upload foto e l'OCR dei PDF scansionati (i PDF con testo nativo restano ok). `GEMINI_OCR_MODEL` override opzionale (default `gemini-2.5-flash`).
